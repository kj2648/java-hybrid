...
The SPF engine (`engines/spf_engine.py`) parses the Jazzer launcher to obtain classpath/target class, generates + compiles a harness into a cache directory, then runs JPF.
  ...
  - Notes:
    - JFO adds `-reload=30`, `-timeout=25`, `--keep_going=0`, `--dedup=true`, `--reproducer_path=<work-dir>/reproducers` and `-artifact_prefix=<work-dir>/artifacts/` unless you override them via `--`.
    - JFO also adds `-close_fd_mask=1` by default to suppress fuzz target stdout noise (override by passing `-close_fd_mask=...` via `--`).
    - Findings are grouped by Jazzer `DEDUP_TOKEN` under `<work-dir>/findings/<token>/<sha1>/` with summaries in `<work-dir>/findings/overview.json` and `<work-dir>/findings/overview.md`.
    - Grouping uses `DEDUP_TOKEN` plus the `Test unit written to .../crash-<sha1>` / `Base64:` lines in `logs/fuzz-*.log` (no crash re-run required).
